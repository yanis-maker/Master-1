...
public:
    ...
    static float dot(Vec3 const &a, Vec3 const &b) {
        ...
        float res;
        res=a.mVals[0] * b.mVals[0] + a.mVals[1] * b.mVals[1] + a.mVals[2] * b.mVals[2]; //Faire le calcul ici et assigner le resultat à res
        return res;
    }
    ...
    static Vec3 cross(Vec3 const &a, Vec3 const &b) {
        ...
        res.mVals[0] = a.mVals[1] * b.mVals[2] - a.mVals[2] * b.mVals[1];
        res.mVals[1] = a.mVals[2] * b.mVals[0] - a.mVals[0] * b.mVals[2];
        res.mVals[2] = a.mVals[0] * b.mVals[1] - a.mVals[1] * b.mVals[0];
        return res;
    }
...
public:
    ...
    Vec3 operator*(const Vec3 &p) {
        ...
        Vec3 res ;
        res[0]= p[0] * (*this)(0,0) + p[1] * (*this)(1,0) + p[2] * (*this)(2,0); //Faire le calcul ici et assigner le resultat à res
        res[1]= p[0] * (*this)(0,1) + p[1] * (*this)(1,1) + p[2] * (*this)(2,1); 
        res[2]= p[0] * (*this)(0,2) + p[1] * (*this)(1,2) + p[2] * (*this)(2,2);
        return res;
    }
    ...
    Mat3 operator*(const Mat3 &m2) { // calcul du produit matriciel m1.m2
        ...
        for(int i=0;i<3;i++){
            for(int j=0;j<3;j++){
                res(i,j)=(*this)(i,j) * m2(i,j) + (*this)(i,j+1) * m2(i+1,j) + (*this)(i,j+2) * m2(i+2,j);
            }
        }
        return res;
    }
